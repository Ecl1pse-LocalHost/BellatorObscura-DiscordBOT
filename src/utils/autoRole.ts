...
async function getTargetRole(guild: any) {
  let targetRole = null;
  ...
  if (TARGET_ROLE_ID && TARGET_ROLE_ID.length > 5) {
    ...
    if (targetRole) {
      if (DEBUG_LOGGING) console.log(`✅ Found target role by ID: ${targetRole.name} (${TARGET_ROLE_ID})`);
      return targetRole;
    }
  }
...
function canAssignRole(botMember: any, targetRole: any) {
...
async function handleMemberVerification(oldMember: any, newMember: any) {
  ...
  if (oldMember.pending && !newMember.pending) {
    if (DEBUG_LOGGING) console.log('🎉 Member completed rules screening!');
...
// Export the handler for use in main bot file
export { handleMemberVerification };